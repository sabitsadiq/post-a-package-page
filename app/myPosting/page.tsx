    ...
    return (
      <tr key={obj.id}>
        ...
        <td className="px-6 py-2 whitespace-nowrap">
          <button className={`bg-[${btnColor[obj.status.toLowerCase()]}] opacity-80 rounded-3xl px-5 py-2`}>{obj.status}</button>
        </td>
        <td className="flex justify-end px-6 py-2 whitespace-nowrap relative">
          <BiDotsVertical size={17} className={`z-10 mr-5 cursor-pointer ${isLastRow === index ? 'lastRow' : ''} ${isSmallScreen && isLastRow === index ? 'mobileLastRow' : ''}`} onClick={() => toggledropDown(obj.id)} />
